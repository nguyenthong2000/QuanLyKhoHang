...
class Kho(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.r = 0
        self.label_title = ["Mã Kho", "Tên Kho", "Địa chỉ"]
        ...
        # Tiêu đề bảng
        self.lb_title = QtWidgets.QLabel("Kho")
        ...
        # Label Mã Kho
        self.lb_ma = QtWidgets.QLabel("Mã Kho")
        self.lb_ma.setStyleSheet("font-size: 14px")

        # Label Tên Kho
        self.lb_ten = QtWidgets.QLabel("Tên Kho")
    ...
    def HienThi(self):
        ...
        for data in loaddata:
            ...
            self.tableWidget.setItem(i, 1, QtWidgets.QTableWidgetItem(data["tenkho"]))
    ...
    def Them(self):
        """Thêm dữ liệu"""
        data = {"tenkho": self.le_ten.text(), "diachi":self.le_diachi.text()}
    ...
    def Sua(self):
        """Sửa dữ liệu"""
        if self.le_ma.text():
            collection.update_one({"_id": ObjectId(self.le_ma.text())},{"$set":{"tenkho":self.le_ten.text(), "diachi":self.le_diachi.text()}})
            self.HienThi()
        else:
    ...
    def TimKiem(self):
        ...
        if self.cbb_timkiemtheo.currentIndex() == 0:
            ...
            loaddata = collection.find({"tenkho": self.le_thanhtimkiem.text()})
        else:
